...
private let audioLog = os.Logger(
    subsystem: Bundle.main.bundleIdentifier ?? "com.joshpointer.flapjax",
    category: "FlapJaxAudio"
)

public class FlapJaxAudioModule: Module {
    ...
    private lazy var engineQueue: DispatchQueue = {
        let q = DispatchQueue(label: "flapjax.audio.engine", qos: .userInteractive)
        q.setSpecific(key: FlapJaxAudioModule.queueKey, value: ())
        return q
    }()
