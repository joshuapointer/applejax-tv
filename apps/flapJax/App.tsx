...
import FlapJaxAudio from './modules/flapjax-audio';
...
export default function App() {
  ...
  const ensureSocket = useCallback(() => {
    ...
    sock.on('error', (err: Error) => {
      console.warn('[flapJax] udp socket error:', err.message);
    });
  ...
  useEffect(() => {
    const pcmSub = FlapJaxAudio.addListener('pcm', ({ data, rms: r }) => {
      rmsRef.current = r;
      ...
      if (nowMs - lastDiagAtRef.current >= 1000) {
        console.log(
          `[flapJax] events=${pcmEventCountRef.current} sent=${sentChunksRef.current} dropped=${droppedRef.current} rms=${r.toFixed(4)}`
        );
        ...
        sock.send(datagram, 0, datagram.length, target.port, target.host, (err: Error | null) => {
          if (err) {
            ...
            if (sentChunksRef.current % 1000 === 0) {
              console.warn('[flapJax] udp send error:', err.message);
            }
          }
        });
        sentChunksRef.current += 1;
      } catch (e) {
        console.warn('[flapJax] pcm send error:', e);
        droppedRef.current += 1;
      }
    });
    const stateSub = FlapJaxAudio.addListener('state', ({ state, error }) => {
      console.log('[flapJax] state event:', state, error ?? '');
      setMode(state);
  ...
  const startMic = useCallback(async () => {
    try {
      setStarting(true);
      console.log('[flapJax] startMic called');
      await FlapJaxAudio.startMic();
      console.log('[flapJax] startMic resolved');
    } catch (e) {
      console.error('[flapJax] startMic error:', e);
  ...
  const stopMic = useCallback(async () => {
    try { await FlapJaxAudio.stopMic(); } catch {}
  }, []);
  ...
  const pickAndPlay = useCallback(async () => {
    try {
      ...
      console.log('[flapJax] playFile:', asset.name, asset.uri);
      setFilename(asset.name ?? 'file');
      setStarting(true);
      await FlapJaxAudio.playFile(asset.uri);
      console.log('[flapJax] playFile resolved');
    } catch (e) {
      console.error('[flapJax] playFile error:', e);
  ...
  const stopFile = useCallback(async () => {
    try { await FlapJaxAudio.stopFile(); } catch {}
  }, []);
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        <Text style={styles.title}>flapJax</Text>
