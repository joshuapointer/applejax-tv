...
Personal-use Apple TV (tvOS) music visualizer built on libprojectM. Pairs with
the **flapJax** iPhone companion app over UDP for real-time audio streaming.

The name riffs on Apple Jacks (the cereal) since this project is a Milkdrop-
descended visualizer — pour the milk, watch the cereal dance. flapJax is the
matching breakfast pun.
...
Expected output: `apps/appleJax/Frameworks/libprojectM.xcframework/` containing `tvos-arm64/` and `tvos-arm64-simulator/` (or `tvos-x86_64-simulator/` on Intel hosts).

Verify with:

```bash
plutil -p apps/appleJax/Frameworks/libprojectM.xcframework/Info.plist | grep -E 'SupportedPlatform|LibraryPath'
```

Clean rebuild: `./apps/appleJax/scripts/build-libprojectm-xcframework.sh --clean`
...
Then sync into `apps/appleJax/Resources/presets/`:
...
├── README.md
├── AGENTS.md
├── project.yml                 # XcodeGen spec → appleJax.xcodeproj
├── appleJax/                   # App source
├── Resources/                  # Bundled presets (gitignored, sync'd)
...
- **No real AirPlay audio visualization** — tvOS third-party apps cannot tap system audio. Workaround: pair the iPhone (flapJax) and stream PCM over UDP, or visualize synthetic beat-grid PCM driven by track metadata for Apple Music playback.
