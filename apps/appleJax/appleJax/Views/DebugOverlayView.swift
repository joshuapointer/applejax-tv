...
/// release entirely. Polls `FlapJaxReceiver.debugSnapshot()` at 10 Hz; the receiver
/// keeps a small lock-protected ring of recent entries so this read is cheap.
struct DebugOverlayView: View {
    @EnvironmentObject private var appState: AppState
    @State private var snapshot: [FlapJaxReceiver.DebugDatagram] = []
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 8) {
                Circle()
                    .fill(appState.flapJaxClientConnected ? Color.green : Color.red)
        ...
        .onReceive(timer) { _ in
            snapshot = appState.flapJaxReceiver?.debugSnapshot() ?? []
        }
    }
