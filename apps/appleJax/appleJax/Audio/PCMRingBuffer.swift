...
final class PCMRingBuffer {
    ...
    func write(_ samples: UnsafePointer<Float>, frameCount: Int) -> Int {
        ...
        buffer.advanced(by: writePos).assign(from: samples, count: firstChunk)
        if secondChunk > 0 {
            buffer.assign(from: samples.advanced(by: firstChunk), count: secondChunk)
        }
    ...
    func read(into destination: UnsafeMutablePointer<Float>, maxFrames: Int) -> Int {
        ...
        destination.assign(from: buffer.advanced(by: readPos), count: firstChunk)
        if secondChunk > 0 {
            destination.advanced(by: firstChunk).assign(from: buffer, count: secondChunk)
        }
