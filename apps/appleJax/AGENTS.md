...
**appleJax** — a personal-use Apple TV (tvOS 26) music visualizer built on libprojectM. Swift 5 / SwiftUI app using Metal for presentation and OpenGL ES 3.0 (off-screen) for projectM rendering. Milkdrop presets at 4K 60fps. Not published; personal use only.

Pairs with the **flapJax** iPhone companion app (`apps/flapJax/`) which streams real-time PCM audio over UDP — the workaround for the tvOS limitation that third-party apps cannot tap AirPlay/system audio.
...
├── Audio/             # AudioSource protocol, AudioController, PCMRingBuffer, MusicKit + procedural sources, FlapJaxReceiver (UDP)
├── Input/             # Siri Remote → InputCommand → handler
├── Presets/           # PresetLibrary (file-based .milk preset management)
├── Util/              # BPMEstimator
├── Views/             # SwiftUI views (RootView, OverlayView, PresetBrowser, SourcePicker, MusicBrowser, WaitingForClientView)
...
1. **Audio in**: `AudioSource` → `PCMRingBuffer` (lock-free ring buffer, 8192 frames). Default source on this app is `FlapJaxReceiver`, which receives UDP datagrams from the flapJax iPhone app.
...
| **FlapJaxReceiver** | POSIX UDP socket on :9999 | Receives PCM from the flapJax iPhone app |

### C interop

- **Bridging header**: `appleJax/Visualizer/ProjectMBridge.h` — imports projectM C API + defines `applejax_gl_load_proc`
...
### Wire protocol (flapJax → appleJax)

UDP datagrams on port 9999:

| Bytes | Field           | Notes                                     |
|------:|-----------------|-------------------------------------------|
| 4     | Magic           | ASCII `APJX` (suite-level)                |
| 4     | Sample count    | `uint32` little-endian                    |
| N     | PCM payload     | `Float32` little-endian, mono, 22050 Hz   |

URL scheme `applejax://<host>:<port>` is broadcast as a QR code on the TV for one-tap pairing from the iPhone app.
...
# Clean rebuild:
./apps/appleJax/scripts/build-libprojectm-xcframework.sh --clean
```
...
### Xcode project
Generated via XcodeGen from `project.yml`. Open `appleJax.xcodeproj` in Xcode. Simulator code-signing is disabled (`CODE_SIGNING_ALLOWED[sdk=appletvsimulator*]: NO`).
...
- **Logging**: Custom `Logger` in `App/Logger.swift` — use domain-specific loggers (`logger`, `renderLogger`, `audioLogger`); subsystem is `com.joshpointer.applejax`
...
- **No system audio tap on tvOS** — third-party apps cannot capture AirPlay/system audio. flapJax UDP feed, MusicKit metadata, or `ProceduralPCMGenerator` are the available sources.
